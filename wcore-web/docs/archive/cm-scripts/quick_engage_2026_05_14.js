...
const targets = [
  {
    label: '@Creatooors fragmented history',
    url: 'https://x.com/Creatooors/status/2055034939725218306',
    id: '2055034939725218306',
    message: 'Persistent user state is the right framing. Most tools remember nothing about your history across chains and wallets, so every session starts from zero context.',
  },
  {
    label: '@ahsan547m fragmented apps',
    url: 'https://x.com/ahsan547m/status/2054623112332025987',
    id: '2054623112332025987',
    message: 'Agreed. The projects that win long term will be the ones that reduce the mental overhead of managing wallets, chains and logins, not the ones adding more features.',
  },
];
...
http.get('http://127.0.0.1:9222/json', (res) => {
  ...
  res.on('end', () => {
    ...
    async function post(target) {
      ...
      await wait(2000);
      const bodyText = await evalJs('document.body.innerText.toLowerCase()');
      if (bodyText.includes('wcore')) { console.log('Already replied, skipping.'); return; }
      ...
      if (!parsed) { console.log('Reply button not found.'); return; }
