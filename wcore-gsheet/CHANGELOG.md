...
## 2026-08-10 — v4.16.65 : staking Cosmos délégué fiable

- Les scans Web Cosmos exposent les composantes déléguées, en déliaison et récompenses avec leur état de complétude.
- Un résultat staking incomplet ne remplace plus le dernier total natif sain dans le cache Google Sheets.
...
## 2026-08-10 - Fallback m�tadonn�es SVM (CWIF + micro-holdings)

- **Web** : propagation symbol/
ame de la source de prix accept�e ; cache prix enrichi ; auto-gu�rison des entr�es Redis sans identit� ; orceRefresh skipe le cache prix SVM.
- **Apps Script** : fallback m�tadonn�es march� sur esolve() ; placeholders sanitiz�s � vide ; pas d��crasement des m�tadonn�es canoniques/Metaplex.
- **GSheet** : CWIF/catwifhat visible dans Layer3 - Solana ; prix et identit� stables.
- **Scam detector** : signal dust r�duit � poids 1 (micro-holdings r�els comme CWIF ne sont plus filtr�s).
- **Cross-runtime** : placeholders mint-prefix align�s Web = Sheet (blank) ; tags canonical/metaplex/market/pc pour retry Metaplex.
- **Prod** : master pouss�, API Railway d�ploy�e, clasp push d�ploy�.
