...
- `Portefeuille Crypto!U2` batch-enqueue le bloc CEX crypto : `BITPANDA_CRYPTO`
  (crypto seul depuis v4.15.115), `BINANCE`, `BITFINEX`, `BYBIT`, `COINBASE`,
  `OKX` en une seule action utilisateur. Statut en `AD2`.

## Triggers

Bitfinex ne fait pas partie de la rotation relay. `WCORE_AUTO_HEAL` maintient exactement un trigger horaire `UPDATE_BITFINEX_SPOT`. Les refresh manuels passent par la queue one-shot (voir [l'architecture CEX](cex-sync.md)); le watchdog legacy reste desactive.
