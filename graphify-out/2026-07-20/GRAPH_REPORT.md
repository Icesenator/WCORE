# Graph Report - graphify-input  (2026-07-20)

## Corpus Check
- 341 files · ~366,025 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4318 nodes · 5734 edges · 300 communities (280 shown, 20 thin omitted)
...
- `registerGmOnchainRoutes()` --references--> `GmOnchainBodySchema`  [EXTRACTED]
  web-api/src/gamification/gm-onchain.ts → web-api/src/schemas.ts
...
Cohesion: 0.11
Nodes (44): DIAG_WEB_SCAN_LAST_ERROR(), DIAG_WEB_SCAN_STATUS(), LIVE_PROBE_WEB_SCAN_CHAIN(), _webScanAcquireAdmission_(), _webScanAcquireAdmissionV2_(), _webScanAllowed_(), _webScanAssetFromNative_(), _webScanAssetFromToken_() (+36 more)

### Community 7 - "gsheet.ts"
Cohesion: 0.06
Nodes (60): token(), CHAINBASE_RPC_TIMEOUT_MS, ChainbaseLiquidityStatus, ChainbaseStaking, CONFIG_PATHS, decodeUint256At(), encodeAddress(), encodeUint256() (+52 more)
...
Cohesion: 0.06
Nodes (52): fetchFxRate(), finalizeDeFiAssets(), hasUnfinalizedDeFiAssets(), ScanJob, scanJobs, startJobCleanup(), resolveScanChainLimit(), scanPlugin() (+44 more)

### Community 11 - "crypto-listing-service.ts"
Cohesion: 0.09
Nodes (27): CanonicalCryptoService, CanonicalCryptoServiceDeps, CmcListingsPayload, CryptoListingRow, CryptoListingSnapshot, CryptoServiceUnavailableError, hasContiguousRanks(), isCompleteSnapshot() (+19 more)
...
Cohesion: 0.17
Nodes (14): toCryptoMarketCapRow(), toStockMarketCapRow(), GsheetStockPortfolioRow, GsheetStockPortfolioSnapshot, buildGsheetStockPortfolioSnapshot(), BuildGsheetStockPortfolioSnapshotInput, holdingForRankedRow(), NormalizedHolding (+6 more)
...
- **Why does `scanPlugin()` connect `scan-utils.ts` to `server.ts`?**
  _High betweenness centrality (0.001) - this node is a cross-community bridge._
- **Why does `verifyWalletHints()` connect `wallet-hints.ts` to `gsheet.ts`?**
  _High betweenness centrality (0.000) - this node is a cross-community bridge._
- **Why does `cexPlugin()` connect `cex.ts` to `server.ts`, `stock-relay.ts`?**
  _High betweenness centrality (0.000) - this node is a cross-community bridge._
