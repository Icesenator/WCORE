...
- **Trigger** : `INSTALL_BITPANDA_SYNC_TRIGGER()` installe `UPDATE_BITPANDA_SPOT()` toutes les heures + `BITPANDA_REFRESH_WATCHDOG()` chaque minute. Vérifier `BITPANDA_SYNC_STATUS()` après exécution.
- **Checkboxes refresh CEX (v4.15.81)** : `Action Rebalancing!Z1` rafraichit uniquement `Bitpanda Spot Stocks` + `Bitpanda Spot Fiat`. `Portefeuille Crypto!AC2` rafraichit `Bitpanda Spot Crypto` + `Bitpanda Spot Fiat` + `Binance Crypto`. Ne pas remettre `AC2` dans `Action Rebalancing` ni `Portefeuille Crypto Details`.
...
- **Refresh manuel CEX uniforme (v4.15.92+)** : cocher `A1` sur un onglet CEX ecrit `REQUEST: ...` en `B1`; le watchdog central `BITPANDA_REFRESH_WATCHDOG()` traite la demande; succes = `B1` timestamp final + rows timestamp identique; `BUSY` = `REQUEST: BUSY retry ...` conserve la demande pour le cycle suivant; erreur = diagnostic visible en `B1`.
...
- **Architecture commune CEX** : voir `docs/cex-sync.md` (flux uniforme `B1=REQUEST`, watchdog central, relais `cex-relay`, helpers `CEX_*` dans `35_BITPANDA_SYNC.gs`, integration `Portefeuille Crypto Details`).
...
## Refresh horaire central CEX (v4.15.98)

- **`CEX_HOURLY_REFRESH()`** (`35_BITPANDA_SYNC.gs`) met à jour les 6 CEX (Bitpanda/Binance/Bitfinex/Bybit/Coinbase/OKX), chaque update protégé indépendamment (un CEX en erreur ne bloque pas les autres).
- **Garanti par l'auto-heal** : `WCORE_AUTO_HEAL` installe `CEX_HOURLY_REFRESH` (trigger 1h) et SUPPRIME les anciens triggers horaires individuels (`UPDATE_BITPANDA_SPOT`, `UPDATE_BINANCE_SPOT`, `UPDATE_BITFINEX_SPOT`, `UPDATE_BYBIT_SPOT`) devenus redondants. Spec bumpée à `v4.15.98:...:cexHourlyRefresh`.
- **Checkboxes refresh (mise à jour)** : `Action Rebalancing!Z1` -> `UPDATE_BITPANDA_STOCKS_FIAT()` (`CEX - Bitpanda Stocks` + `CEX - Bitpanda Fiat`). `Portefeuille Crypto!AC2` -> `UPDATE_BITPANDA_CRYPTO_FIAT()` + Binance + Bitfinex + Bybit + Coinbase + OKX.
...
- **Fix permanent self-heal (v4.15.103, 2026-06-29)** : `_bpEnsureCexTriggers_()` dans `35_BITPANDA_SYNC.gs` **delete + recreate systématiquement** les 5 triggers CEX (`BITPANDA_REFRESH_WATCHDOG`, `BINANCE_REFRESH_WATCHDOG`, `BITFINEX_REFRESH_WATCHDOG`, `BYBIT_REFRESH_WATCHDOG`, `CEX_HOURLY_REFRESH`) depuis chaque `*_ON_EDIT` (BITPANDA/BINANCE/BITFINEX/BYBIT). Un trigger `count=1` n'est pas garanti vivant (peut être sous OAuth stale) — la seule façon de capturer le fresh auth user est de delete+recreate, pas create-if-missing. Appelé après le check A1/AC2, dans un `try/catch` silencieux pour ne pas bloquer le flux principal. Couvre l'angle mort du v4.15.61 (auto-heal lui-même mort). `BP_REINSTALL_CEX_TRIGGERS()` exposé en user-facing pour run depuis l'éditeur.
- **GAS restriction `everyMinutes`** : n'accepte que `{1, 5, 10, 15, 30}`. Pour 1h, utiliser `everyHours(1)` (table `_BP_CEX_TRIGGERS_TO_HEAL` distingue `unit: "minutes"` vs `unit: "hours"`).
- **Validation self-heal** : `validate:static` OK 2937 functions, B1 `CEX - Bitpanda Crypto` repassé à `2026-06-29 20:43:10` (data `updated_at=20:43`) après click AC2 + run manuel `BP_REINSTALL_CEX_TRIGGERS`. Plus aucun stuck watchdog permanent si l'user clique régulièrement n'importe quel A1 CEX.
