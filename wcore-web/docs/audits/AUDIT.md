...
- [x] **RESOLVED 2026-08-05, preuve recroisee 2026-08-09.** L'engine TON choisit `opts.sources` avant les sources construites par defaut et partage ce jeu pour le natif et les jettons. `ton.test.ts` injecte une source a 3 EUR, exige qu'elle soit consultee et que le prix atteigne le resultat. Ignorer `opts.sources` fait echouer exactement cette garde.
- [~] **PARTIEL 2026-08-09.** Les trois variantes non-EVM `empty:v2:*` (Cosmos/SVM/TON) passent par l'entree web-only `emptyWalletV2`; motif strictement preserve et appels au registre verrouilles par test. L'inventaire recroise trouve encore des familles directes (`meta`, `native`, `token`, `bal_cache`, staking Cosmos, IBC, jettons TON, RealT): la case reste ouverte jusqu'a leur migration sans invalidation de cache.
