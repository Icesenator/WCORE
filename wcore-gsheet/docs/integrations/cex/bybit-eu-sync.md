...
- Transitoire (timeout Spreadsheets / quota / `BUSY`) : `B1 = RETRY n/2: ...`; le job est requeue (max 2) dans le meme worker apres `Utilities.sleep(2000)`. Si la queue reste non vide a la sortie du worker, `_cexEnsureManualWorkerTrigger_(5000)` le reprogramme a +5s.
...
`WCORE_AUTO_HEAL` installe `UPDATE_CEX_RELAY_ROTATION` toutes les 15 minutes. Bybit est le deuxieme tour, soit environ une mise a jour par heure. Les demandes manuelles passent par `CEX_MANUAL_REFRESH_WORKER`; les watchdogs restent `LEGACY_DISABLED`. `INSTALL_BYBIT_SYNC_TRIGGER()` est desactive et ne cree aucun trigger.
...
Voir [l'architecture commune CEX](cex-sync.md).
...
- [Architecture commune CEX](cex-sync.md) - architecture commune des 6 connecteurs CEX.
