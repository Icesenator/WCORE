...
(async () => {
  ...
  try {
    ...
    try {
      await renderPage.goto(`file://${TMP_HTML.replace(/\\/g, "/")}`, { waitUntil: "load" });
    ...
    console.log(`SVG written: ${SVG_PATH} (${Buffer.byteLength(svg)} bytes, ${W}x${H})`);
    console.log(`PNG written: ${PNG_PATH} (${W}x${H})`);
  } finally {
    try {
