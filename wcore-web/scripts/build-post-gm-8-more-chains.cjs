// Build script for the X post: "8 more GM chains. Again."
// Celebrates the second 8-chain GM factory batch (2026-06-11):
// Gravity, Merlin, Manta Pacific, Taiko Alethia, Plasma, HashKey, Hemi, HyperEVM.
// 72 GM chains live now (was 64 before this session).
// Output: apps/web/public/wcore-post-gm-8-more-chains.svg + .png
...
const OUT_NAME = "wcore-post-gm-8-more-chains";
const W = 1200;
const H = 675;
...
const NEW_CHAINS = [
  ...
  { key: "merlin", label: "Merlin" },
  { key: "manta_pacific", label: "Manta Pacific" },
  { key: "taiko_alethia", label: "Taiko" },
...
function logoPathFor(key) {
  ...
  // .svg before .png: HEMI.png is a stale mascot, HEMI.svg is the official orange mark.
  const candidates = [
    `${base}.webp`, `${base}.svg`, `${base}.png`, `${base}.jpg`, `${base}.jpeg`, `${base}.ico`,
  ];
...
function cell(chain, i, x, y, w, h) {
  ...
  const labelSize = chain.label.length > 9 ? 18 : 22;
  const icon = logo
  ...
  return `
    <g>
      ...
      ${icon}
      <text x="${labelX}" y="${labelY}" class="font white" font-size="${labelSize}" font-weight="900">${escapeXml(chain.label)}</text>
    </g>`;
}
...
function buildSvg() {
  ...
  // Title
  parts.push(`<text x="72" y="220" class="font white" font-size="58" font-weight="950" letter-spacing="-2.6">8 more GM chains. Again.</text>`);
  ...
  parts.push(`<g transform="translate(72 ${pillY})">
    ...
    <text x="221" y="21" text-anchor="middle" class="font white" font-size="13" font-weight="800" letter-spacing="1.0">One template</text>
    <rect x="304" y="0" width="142" height="32" rx="16" fill="#0c1218" stroke="#2b3340"/>
    <text x="375" y="21" text-anchor="middle" class="font white" font-size="13" font-weight="800" letter-spacing="1.0">BTC L2s in</text>
  </g>`);
