...
- **Top 300 actions (2026-06-13)** : `src/34_TOP_MARKETCAP.gs` alimente `Google Finance` depuis `companiesmarketcap.com` (`?download=csv`) et reconstruit `Action Rebalancing`. Documentation détaillée : `docs/reference/top-marketcap-google-finance.md`.
...
- **Connecteur Bitpanda (2026-06-13)** : `src/35_BITPANDA_SYNC.gs` remplace progressivement SyncWith via l'API Bitpanda officielle. Documentation : `docs/integrations/cex/bitpanda-sync.md`.
...
- **Triggers auto (v4.16.34)** : Bitpanda utilise deux triggers horaires dédiés, `UPDATE_BITPANDA_SPOT` et `UPDATE_BITPANDA_STOCKS_FIAT`. `INSTALL_BITPANDA_SYNC_TRIGGER()` réinstalle ces deux triggers; `BITPANDA_REFRESH_WATCHDOG()` reste `LEGACY_DISABLED`. Bitfinex et Kraken gardent chacun leur trigger horaire indépendant. Les providers relay Binance/Bybit/Coinbase/OKX passent par `UPDATE_CEX_RELAY_ROTATION` toutes les 15 minutes.
...
- **Connecteur Binance (2026-06-13)** : `src/36_BINANCE_SYNC.gs` alimente `Binance Crypto` via relais Railway. Documentation : `docs/integrations/cex/binance-sync.md`.
...
- **Binance manuel** : `Binance Crypto!A1` refresh seulement Binance. `Portefeuille Crypto!AC2` refresh le bloc CEX crypto complet (`Bitpanda Spot Crypto`, `Bitpanda Spot Fiat`, `Binance Crypto`, `Bitfinex Crypto`, `Bybit Crypto`). Les watchdogs individuels sont legacy; l'auto-heal supprime les anciens triggers relay individuels et garde uniquement la rotation 15 minutes.
- **Architecture commune CEX** : voir `docs/integrations/cex/cex-sync.md` (queue one-shot `CEX_MANUAL_JOB_QUEUE` + worker lease, locks par connecteur `CEX_ACQUIRE_LOCK`, relais `cex-relay`, helpers `CEX_*` dans `35_BITPANDA_SYNC.gs`, integration `Portefeuille Crypto Details`).
...
- **Connecteur Bitfinex (2026-06-14)** : `src/37_BITFINEX_SYNC.gs` alimente `Bitfinex Crypto` via l'API officielle v2. Documentation : `docs/integrations/cex/bitfinex-sync.md`.
...
- **Refresh manuel** : `Bitfinex Crypto!A1` refresh seulement Bitfinex. `Portefeuille Crypto!AC2` inclut Bitfinex dans le bloc CEX commun via la queue manuelle.

## ByBit EU Sync

- **Connecteur Bybit EU (2026-06-15)** : `src/38_BYBIT_SYNC.gs` (version `4.15.95`) alimente `Bybit Crypto` via le relais Railway multi-CEX `cex-relay`. Documentation : `docs/integrations/cex/bybit-eu-sync.md`.
...
- **Refresh manuel et auto** : `CEX - Bybit!A1` écrit `B1=QUEUED: ...` et pousse un job `BYBIT` dans la queue one-shot (`CEX_MANUAL_REFRESH_WORKER`). Le refresh auto Bybit est un tour de `UPDATE_CEX_RELAY_ROTATION` toutes les 15 minutes, soit environ une fois par heure; l'ancien trigger Bybit individuel est non canonique.
...
- **Connecteur Coinbase (2026-06-15)** : `src/39_COINBASE_SYNC.gs` alimente `CEX - Coinbase` via le relais Railway `cex-relay`. Documentation : `docs/integrations/cex/coinbase-sync.md`.
...
- **Connecteur OKX (2026-06-15)** : `src/40_OKX_SYNC.gs` alimente `CEX - OKX` via le relais Railway `cex-relay`. Documentation : `docs/integrations/cex/okx-sync.md`.
...
## Rotation CEX + queue one-shot (v4.16.34)

- **`UPDATE_CEX_RELAY_ROTATION()`** (`44_CEX_BULK.gs`) appelle exactement un provider relay par invocation, dans l'ordre Binance, Bybit, Coinbase, OKX. Son trigger unique `everyMinutes(15)` donne environ un refresh/heure/provider sans exécution séquentielle multi-provider.
- **Curseur robuste** : `CEX_RELAY_ROTATION_CURSOR` avance dans ScriptProperties sous ScriptLock avant l'appel provider. Une erreur ou un timeout ne bloque donc pas les providers suivants. `WCORE_AUTO_HEAL` garde exactement une rotation et supprime les anciens triggers bulk/individuels relay; `UPDATE_CEX_RELAY_ALL()` reste manuel/fallback.
...
- **Tests de garde** : `tests/cex-relay-rotation.test.js`, `tests/cex-refresh-load-guard.test.js` et `tests/action-rebalancing-direct-refresh.test.js` verrouillent rotation/curseur, queue/lease/batch/retry, watchdogs désactivés, onEdit no-op, worker 1 min et re-pulse `BUSY:CEX`.
...
- **Fix permanent self-heal (v4.16.34)** : `_bpEnsureCexTriggers_()` réinstalle les triggers canoniques avec une autorisation fraîche: Bitpanda x2, Bitfinex et Kraken horaires, rotation relay 15 minutes et worker manuel 1 minute. Il supprime les anciens bulk, triggers relay individuels et watchdogs legacy. Il n'est pas appelé depuis les `*_ON_EDIT` : uniquement via `BP_REINSTALL_CEX_TRIGGERS()` ou `WCORE_CEX_TRIGGER_CLEANUP_FORCE()` depuis l'éditeur.
- **GAS restriction cadences** : `everyMinutes` n'accepte que `{1, 5, 10, 15, 30}`; `everyHours` que `{1, 2, 4, 6, 8, 12}`. La rotation relay utilise 15 minutes; les quatre tours donnent environ une heure par provider.
