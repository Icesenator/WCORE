...
- **Consolidation des stables (v4.16.46)** : `BITPANDA_SYMBOL_ALIASES` canonise le symbole **avant** l'agrégation (`_bpWalletRow_` → `_bpCanonicalSymbol_`, puis `_bpPushUniqueRow_` **somme** par symbole). Un alias fusionne donc réellement deux lignes en une. Consolidations en place : `USDC → USDT` et `EURCV → EURC` (EUR CoinVertible, la stable euro de Société Générale, que Bitpanda expose sous son propre ticker). Sans l'alias, la balance ne correspondait à aucune ligne du portefeuille.
- **Colonne `Vérif` des onglets CEX** : `V`/`X` ne juge ni le prix ni la balance. La formule (`_cexBuildVerifFormula_`) teste seulement si le couple (symbole, nom d'onglet) existe dans `Portefeuille Crypto Details` (colonnes C et E). Un `X` signifie donc « ce symbole n'est rattaché à aucune position suivie », typiquement un ticker que le connecteur n'a pas canonisé — pas une erreur de valorisation.
- **Triggers CEX (v4.15.120)** : l'auto-heal installe les refreshs Bitpanda/Bitfinex/Kraken horaires et `UPDATE_CEX_RELAY_ROTATION` toutes les 15 min. L'ancien `CEX_HOURLY_REFRESH` central et les watchdogs connecteur sont legacy. Vérifier `BITPANDA_SYNC_STATUS()` après exécution.
...
- **Refresh manuel** : `CEX - Bybit!A1` écrit `B1=QUEUED: ...` et pousse un job `BYBIT` dans la queue one-shot (`CEX_MANUAL_REFRESH_WORKER`). `UPDATE_CEX_RELAY_ROTATION()` tourne toutes les 15 min mais traite un seul des quatre connecteurs relais par execution, soit environ un refresh Bybit par heure; les triggers Bybit individuels sont legacy.
...
- **Refresh CEX distribue (v4.15.120)** : `WCORE_AUTO_HEAL` installe `UPDATE_BITPANDA_SPOT`, `UPDATE_BITPANDA_STOCKS_FIAT`, `UPDATE_BITFINEX_SPOT` et `UPDATE_KRAKEN_SPOT` toutes les heures, plus `UPDATE_CEX_RELAY_ROTATION` toutes les 15 min. La rotation traite un seul des quatre connecteurs Binance/Bybit/Coinbase/OKX par execution, soit environ une fois par heure chacun. L'ancien `CEX_HOURLY_REFRESH` central est supprime. Après changement de cadence, run `WCORE_CEX_TRIGGER_CLEANUP_FORCE()` une fois depuis l'éditeur.
...
- **Fix permanent self-heal (v4.15.103, mis à jour v4.15.120)** : `_bpEnsureCexTriggers_()` dans `35_BITPANDA_SYNC.gs` **delete + recreate** les triggers CEX canoniques avec une autorisation fraîche et supprime le central/watchdogs legacy. Il n'est pas appelé depuis les `*_ON_EDIT` (trop lent pendant un clic) : uniquement via `BP_REINSTALL_CEX_TRIGGERS()` ou `WCORE_CEX_TRIGGER_CLEANUP_FORCE()` depuis l'éditeur. Le principe delete+recreate reste nécessaire car `count=1` ne prouve pas qu'un trigger tourne.
...
### Gotchas configs de chaînes — session 2026-08-02

- **`dist/` peut diverger de `src/` sans que rien ne le signale.** `wcore-gsheet/src/*.gs` est la source de vérité, mais c'est `dist/` (package `@wcore/chains`) que consomment l'API et le web : `wcore-web/packages/core/src/chains/index.ts` réexporte `@wcore/chains` et ne contient aucune chaîne locale. Le 2026-08-02, quatre correctifs déjà committés dans `src` n'avaient jamais été extraits (RPC morts retirés sur BSC et Syndicate Commons, `MAX_LOG_RANGE` sur BSC et Monad, version TON) : la production tournait sur des configs périmées. **Lancer `npm run build:chains` et committer `dist/` dans le même commit que toute modification de config de chaîne.**
- **Un RPC peut répondre selon l'IP d'origine.** `ethereum-rpc.publicnode.com` répond normalement depuis une IP résidentielle et depuis Apps Script (IP Google), mais renvoie HTTP 403 depuis les IP datacenter de Railway, à chaque scan. Comme la config est partagée entre les deux runtimes, le supprimer aurait dégradé Apps Script. La sélection étant ordonnée (`pool.slice(0, maxRpcs)` après un filtre qui préserve l'ordre, cf. `rpc/dispatcher.ts` et `rpc/rpc-health.ts`), **rétrograder l'endpoint en fin de liste plutôt que le supprimer**. Même raisonnement appliqué au thirdweb mort de LayerAI.
- **Ne jamais valider un RPC sur le seul `eth_chainId`.** Vérifier aussi `eth_blockNumber`, `eth_getBalance` et `eth_call`, et comparer le bloc de tête avec un endpoint déjà en place : un écart nul prouve qu'on est bien sur la même chaîne. C'est ce qui a permis d'ajouter `rpc.ankr.com/tac` et `rpc.mainnet.tempo.xyz` en confiance, et de rejeter un candidat AVES dont le nom d'hôte contenait `testnet`.
- **`MAX_LOG_RANGE` est un écart, pas un nombre de blocs.** Le moteur calcule `fromBlock = toBlock - MAX_LOG_RANGE` (`engines/evm-balances.ts`), donc la fenêtre couvre `MAX_LOG_RANGE + 1` blocs. TAC rejette `-32062 Block range is too large` au-delà de 1000 blocs inclusifs : la valeur correcte est **999**, pas 1000. Mesurer la limite réelle par dichotomie avant de la fixer.
- **Chaînes sans aucun RPC vivant (2026-08-02)** : `SYNDICATE_COMMONS` (502 sur `commons.rpc.syndicate.io`, DNS mort sur `rpc.commons.syndicate.io`, `Invalid chain` sur thirdweb) et `AVES_NETWORK` (thirdweb bloqué, aucun candidat mainnet chez chainlist). Elles restent activées pour préserver le cache existant, mais ne peuvent pas être scannées tant qu'un endpoint n'est pas trouvé.
### Gotchas configs de chaînes — session 2026-08-03

- **Régénérer `dist/` ne suffit pas à propager une correction en local.** `@wcore/chains` est déclaré `"file:../../../wcore-gsheet/dist"` dans `wcore-web/packages/core/package.json`, et pnpm le matérialise en **copie figée** dans son store (`node_modules/.pnpm/@wcore+chains@file+..+wcore-gsheet+dist/`), pas en lien vers `dist/`. Après `npm run build:chains`, les tests et builds web continuent donc de lire l'ancienne config jusqu'à un `pnpm install` dans `wcore-web` (rapide, ~2 s, et sans modification du lockfile). Symptôme vécu : `CHAIN_ID` corrigé à `5031` dans `src` et `dist`, mais le test core échouait encore sur `50311`. La CI et Railway installent à froid et ne sont pas concernés, mais **toute vérification locale d'un changement de chaîne exige ce `pnpm install`**.
- **Ne jamais valider un endpoint sur le seul fait qu'il répond.** `rpc.reya-cronos.gelato.digital` était dans le pool mainnet de `REYA` (`1729`) et répondait parfaitement — mais avec le chainId `89346162`, celui du **testnet** Reya Cronos. Un endpoint d'un autre réseau ne dégrade pas le scan, il peut servir des soldes faux. `node tools/verify-chain-ids.mjs` détecte cette classe de bug ; il n'échoue que sur une contradiction avérée et ignore les endpoints injoignables, dont le résultat dépend de l'IP.
- **Supprimer vs rétrograder.** Rétrograder en fin de liste quand l'endpoint est bon mais bloqué (403/408 dépendant de l'IP) : il peut servir depuis Apps Script ou une autre IP. **Supprimer** quand l'endpoint sert une autre chaîne : un mauvais réseau n'est jamais un repli acceptable.
- **Un LCD Cosmos peut répondre sur `blocks/latest` et rien d'autre.** Toujours sonder les routes réellement utilisées par le moteur (`/cosmos/bank/v1beta1/...`, `/cosmos/staking/v1beta1/...`) avant d'ajouter un endpoint. `noble-rest.publicnode.com` renvoie 404 sur toutes les routes de module ; `stride-rest.publicnode.com` renvoie 403 `unsupported platform`. Le failover multi-endpoint se configure via `API.REST_URLS` (tableau ordonné, premier = primaire), pas seulement `REST_URL` — voir `packages/core/src/engines/cosmos.ts`.
- **Les fichiers `wcore-web/packages/core/src/chains/*.ts` sont morts au runtime.** `index.ts` ne réexporte que `@wcore/chains` (`0 local web-only chains`). Ces copies ne servent plus qu'au test Phase 3, et elles ont dérivé (ex. `POLYNOMIAL.ts` porte un `DISABLE_CHAIN: true` sans aucun effet, `BASE.ts` a des endpoints différents). Ne jamais y lire l'état réel d'une chaîne : la vérité est dans `wcore-gsheet/src/*.gs` puis `dist/`.

- **`test:phase3-chains` compare certains `.gs` à `wcore-web/packages/core/src/chains/*.ts`.** Le contrôle ne s'applique qu'aux fichiers portant le marqueur `Phase 3 ... port from wcore-web chain config` (LayerAI en fait partie, TAC et Tempo non). Modifier un de ces `.gs` impose de mettre à jour la copie web, sinon le test échoue en `deepStrictEqual`.
