...
Bitfinex, Bybit, Coinbase, OKX et Kraken. Pour les details specifiques a chaque exchange,
voir [Bitpanda](bitpanda-sync.md), [Binance](binance-sync.md),
[Bitfinex](bitfinex-sync.md), [Bybit EU](bybit-eu-sync.md),
[Coinbase](coinbase-sync.md), [OKX](okx-sync.md) et [Kraken](kraken-sync.md).
...
Refresh automatique: `WCORE_AUTO_HEAL` installe des triggers
...
central `CEX_HOURLY_REFRESH`, les triggers relais individuels et
`UPDATE_CEX_RELAY_ALL` sont supprimes. `UPDATE_CEX_RELAY_ALL` reste
`LEGACY_DISABLED`.
...
- `_cexEnqueueManualJobs_(jobs)` : enqueue BATCH — 1 ecriture
  ...
  ensure du trigger worker. Utilise par `T2` (1 job) et `U2` (7 jobs).
...
- `_cexRunManualJob_(job)` : execute le job; erreurs transitoires
  ...
  (max 2 retries, statut visible `RETRY n/2: ...`). Le job requeue reste dans
  le meme worker apres `Utilities.sleep(2000)`. Si la queue reste non vide a la
  sortie du worker apres un retry, `_cexEnsureManualWorkerTrigger_(5000)`
  reprogramme le worker a +5s.
...
5. Transitoire (timeout Spreadsheets / quota / BUSY) : `B1 = RETRY n/2: ...`;
   le job est requeue et le meme worker continue apres `Utilities.sleep(2000)`.
   Si la queue reste non vide a sa sortie, `_cexEnsureManualWorkerTrigger_(5000)`
   reprogramme le worker a +5s.
...
de 6 min avec marge). Ne PAS revenir au
...
Le watchdog on-chain (B1 pulse) les ignore : leur refresh passe par les triggers CEX canoniques et la queue manuelle.

## Gotchas cles

- Ne JAMAIS reintroduire les watchdogs CEX individuels ni des triggers relay individuels
  (conflits de lock et pression sur la limite de triggers).
